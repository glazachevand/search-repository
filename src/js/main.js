...
function validate(form) {
  if (form.search.value.length < min) {
    showError();
    return false;
  } else {
    return true;
...
function renderSearchResults(result) {
  ...
  if (result.items.length === 0) {
    ...
    document.querySelector('.search-results__title span').textContent = ` (${searchLength} из ${result.total_count})`;
...
function renderSearchItem(item) {
  ...
  if (item.created_at) html +=
    `<div class="repositories-item__date"><time datetime="${item.created_at}">Дата создания: ${new Date(item.created_at).toLocaleDateString("ru-RU")}</time></div>`;
  if (item.updated_at) html +=
    `<div class="repositories-item__date"><time datetime="${item.updated_at}">Дата последнего обновления: ${new Date(item.updated_at).toLocaleDateString("ru-RU")}</time></div>`;
  if (item.homepage) html += `<div class="repositories-item__homepage"><a href="${item.homepage}" target="_blank" >Сайт</a></div>`;
